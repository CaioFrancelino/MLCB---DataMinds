...
# Agente:
# O robô-aspirador que toma decisões sobre onde se mover e quando limpar ou recarregar.

# Ambiente:
# A casa, composta por vários cômodos, sujeira espalhada e uma base de recarga.

# Estado (exemplifique alguns estados possíveis):
# - Robô está no quarto, bateria em 80%, sujeira presente.
# - Robô está na sala, bateria em 30%, sem sujeira.
# - Robô está na cozinha, bateria em 10%, precisa recarregar.

# Ação (exemplifique algumas ações possíveis):
# - Mover para o próximo cômodo.
# - Limpar o cômodo atual.
# - Retornar à base para recarregar.

# Recompensa (sugira algumas recompensas positivas e negativas):
# - +10 por limpar um cômodo com sujeira.
# - -5 por tentar limpar um cômodo já limpo.
# - -20 por ficar sem bateria antes de retornar à base.

# Política (descreva brevemente o que uma política representaria aqui):
# Uma política seria uma estratégia que o robô segue para decidir suas ações, como priorizar cômodos com mais sujeira ou retornar à base antes que a bateria acabe.

# Função de Valor Q (o que ela tentaria estimar neste contexto?):
# A função de valor Q estimaria a recompensa esperada para cada ação possível em um determinado estado, ajudando o robô a escolher a melhor ação para maximizar a eficiência da limpeza.
